...
# else is running as a daemon.
} else {
    my($port_arg) ;
    GetOptions('port|p=i' => \$port_arg)  or die "bad options\n" ;
    $port_arg||= 443 ;
	...
	init ;
	$pid= spawn_generic_server($HTTPS_LISTEN, $LOCK_FH, \&handle_https_request, 0) ;
    }
    print "Port: $port\nPID:  $pid\n" ;
}
...
# was:  ($rcode, @r)= &ftp_command("LIST $path\015\012", '125|150') ;
	...
	if ($rcode==550) {
	    ...
	    print $STDOUT "$HTTP_1_X 301 Moved Permanently\015\012", $NO_CACHE_HEADERS,
...
#$Net::SSLeay::trace= 1 ;
    my $rv= Net::SSLeay::accept($ssl_obj->{SSL}) ;
    Net::SSLeay::die_if_ssl_error("SSL accept() error (rv=[$rv]): $!");


    # SSL connection is now set up.  Handle HTTP request.
    ...
    $ENV{REMOTE_ADDR}= inet_ntoa((unpack_sockaddr_in(getpeername($SS)))[1]) ;
...
# Take the listening socket, the lock filehandle, a code reference, and a
#   timeout as params.  The timeout, in seconds, applies to the daemon process;
#   0 means no timeout.
...
sub spawn_generic_server {
    my($LISTEN, $LOCK_FH, $coderef, $timeout)= @_ ;
	...
	exit(&$coderef($SS, $port)) ;
    }


    # Clean up zombies.  Damn zombies.
    sub REAPER {
	local $! ;
...
sub startproxy {
    ...
    my $encode_prefix= $ENV{HTTP_USER_AGENT}=~ /Chrome/  ? "\x7f"  : "\x01" ;
...
sub js_insertion {
    ...
    return '<script type="text/javascript" src="'
	 ...
	 . "\"></script>\n"
	 . qq(<script type="text/javascript">_proxy_jslib_pass_vars("$base_url_jsq",$p_cookies_are_banned_here,$p_doing_insert_here,$p_session_cookies_only,$p_cookie_path_follows_spec,$p_respect_three_dot_rule,$p_allow_unproxified_scripts,"$RTMP_SERVER_PORT","$default_script_type_jsq","$default_style_type_jsq");</script>\n) ;
}
...
sub skip_image {
    ...
    my($date_header)= &rfc1123_date($now, 0) ;
    print $STDOUT "$HTTP_1_X 406 Not Acceptable\015\012${NO_CACHE_HEADERS}Date: $date_header\015\012\015\012" ;
    goto EXIT ;
}
...
sub redirect_to {
    ...
    print $STDOUT "$HTTP_1_X 302 Moved\015\012", $NO_CACHE_HEADERS,
		  "Date: ", &rfc1123_date($now,0), "\015\012",
		  $other_headers,
...
#		$prefix= $token ;
	...
	#   not ")", or if there's no previous token, then assume it's an object
	#   literal.  Not perfect.
...
function _proxy_jslib_pass_vars(base_url, cookies_are_banned_here, doing_insert_here, SESSION_COOKIES_ONLY, COOKIE_PATH_FOLLOWS_SPEC, RESPECT_THREE_DOT_RULE, ALLOW_UNPROXIFIED_SCRIPTS, RTMP_SERVER_PORT, default_script_type, default_style_type) {
...
function _proxy_jslib_reverse_full_url(url) {
    return _proxy_jslib_parse_full_url(uri_ref)[2] ;
}
...
sub proxify_swf_cpool_info {
    my($in)= @_ ;
    my(@out, $s) ;
    my $n_specials= { connect => [], play => [], URLRequest => [], loaderURL => [], loadPolicyFile => [] } ;
    my $mn_specials= { connect => [], play => [], URLRequest => [], loaderURL => [], loadPolicyFile => [] } ;
    ...
    # Copy through strings, adding the 15 strings we need.
    my $string_count= &get_swf_u30_32($in) ;
    push(@out, &set_swf_u30_32($string_count+14)) ;
    $start_pos= pos($$in) ;
    ...
    push(@out, substr($$in, $start_pos, pos($$in)-$start_pos)) ;
    push(@out, "\x09flash.net\x0aURLRequest\x03url\x0eflash.external\x11ExternalInterface\x15_proxy_jslib_full_url\x04call\0\x0dNetConnection\x09NetStream\x1d_proxy_jslib_full_url_connect\x1a_proxy_jslib_full_url_play\x05alert\x1d_proxy_jslib_reverse_full_url") ;

    # Copy through namespace info, adding the 3 we need.
    my $ns_count= &get_swf_u30_32($in) ;
    push(@out, &set_swf_u30_32($ns_count+3)) ;
    $start_pos= pos($$in) ;
    ...
    push(@out, &set_swf_u30_32($multiname_count+6)) ;
    $start_pos= pos($$in) ;
...
# Add 3 method_info records: "\0\0\0\0", "\x01\0\0\0\0" x 2
...
sub proxify_swf_method_body_infos {
    ...
    # Since length may vary, must calculate length of the code after the jump.
    my($post_construct_ins, $pre_connect_ins, $pre_play_ins, $post_loaderURL_ins,
       $proxify_top_url_ins, $after_jump) ;
    ...
    $proxify_top_url_ins= "\x60" . &set_swf_u30_32($multiname_count+2)
		 . "\x2b\x2c" . &set_swf_u30_32($string_count+6)
    ...
    # Next version not used, as we don't check for NetConnection object.  See notes
    #   in avm2.asm .
#    $after_jump= "\x60" . &set_swf_u30_32($multiname_count+2)
#	       . "\x2b\x2c" . &set_swf_u30_32($string_count+10)
#	       . "\x2b\x46" . &set_swf_u30_32($multiname_count+3) . "\x02" ;
#    $pre_connect_ins= "\x2a\xb2" . &set_swf_u30_32($multiname_count+4)
#		    . "\x12" . &set_swf_s24(length($after_jump)) . $after_jump ;
    $pre_connect_ins= "\x60" . &set_swf_u30_32($multiname_count+2)
		    . "\x2b\x2c" . &set_swf_u30_32($string_count+10)
		    . "\x2b\x46" . &set_swf_u30_32($multiname_count+3) . "\x02" ;

    # jsm-- this is faulty-- see notes in avm2.asm .
    $after_jump= "\x60" . &set_swf_u30_32($multiname_count+2)
	       . "\x2b\x2c" . &set_swf_u30_32($string_count+11)
	       . "\x2b\x46" . &set_swf_u30_32($multiname_count+3) . "\x02" ;
    $pre_play_ins= "\x2a\xb2" . &set_swf_u30_32($multiname_count+5)
		 . "\x12" . &set_swf_s24(length($after_jump)) . $after_jump ;
    ...
    for my $mb (0..$count-1) {
	$pos= pos($$in) ;
	my $method_info= &get_swf_u30_32($in) ;
	...
	($code, $insertions)=
		...
		$post_loaderURL_ins, $pre_coerce_ins_part1, $pre_coerce_ins_part2, $mn_specials, $mb, $alert_ins) ;
...
# $mn_specials is the list of which multinames refer to a name "connect" or "play".
sub proxify_swf_avm2_code {
    my($code, $proxify_top_url_ins, $post_construct_ins, $pre_connect_ins, $pre_play_ins, $post_loaderURL_ins,
       $pre_coerce_ins_part1, $pre_coerce_ins_part2, $mn_specials, $mb, $alert_ins)= @_ ;
...
sub get_swf_multiname {
	...
	foreach my $p (qw(connect play URLRequest loaderURL loadPolicyFile)) {
